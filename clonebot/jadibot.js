      ...
      client[from].ev.on("connection.update", async up => {
        ...
        if (connection === "close") {
          if (reason === DisconnectReason.restartRequired) {
            ...
            return anggazyy.sendMessage(from, { text: "Anda sudah tidak lagi menjadi bot." })
          }
        }
      })
      ...
      client[from].ev.process(async (events) => {
        if (events['messages.upsert']) {
          ...
          for (let msg of upsert.messages) {
            ...
            require("../anggazyy")(client[from], m, store)
          }
        }
      })
...
async function stopjadibot(anggazyy, from) {
  if (!Object.keys(client).includes(from)) {
    return anggazyy.sendMessage(from, { text: "Anda tidak ada di list jadi bot." })
  }
  try {
